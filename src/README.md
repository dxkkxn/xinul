...
* user/ : espace user de l'OS, ils sont compilés seul et intégrés au kernel ensuite.
    * ./*****/ : un programme user est placé dans un dossier et ajouté au Makefile user (APPS_NAME).
    ...
    * ./build/ : outils pour le Makefile.
    * ./Makefile : permet de compiler seulement l'espace user du projet.
* kernel : espace kernel du projet.
    * ./boot/
        ...
        * ./*MACHINE*/ : dossier pour la machine x particulière.
            * default.lds : script de lien du kernel.
            * setup.c : configuration spécifique à la machine (UART, poweroff, …).
        * ./boot.mk : inclu par le Makefile kernel, il informe sur les objets a générer pour le boot.
    * ./Makefile : permet de compiler le kernel sans recompiler l'espace user.
...
* kernel : compile le kernel sans recompiler l'espace user mais prend quand même les programmes user précédaments générés.

#### Simulation

* go : lance le kernel avec Qemu dans le terminal. (Dépendance : all);
* run : lance le kernel avec Qemu dans le terminal et attend la connection avec GDB. (Dépendance : all);
...
* dk-all : compile le kernel dans un container, les fichiers de sortie seront aussi dans `./build/`;
* dk-go : lance le kernel avec Qemu dans un container;
* dk-run : lance un container avec Qemu dans une optique de debug. Qemu attend gdb pour continuer. Le port 1234 de la machine host est mapé avec Qemu.
