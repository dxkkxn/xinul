...
void enter_supervisor_mode()
{
	...
	// On indique que le mode précédent était le mode supervisor.
	// c'est le mode qui sera utilisé après l'instruction mret
	csr_set(mstatus, MSTATUS_MPP & MSTATUS_MPP_S);

	// ON désactive les interruption supervisor pour l'initialisation
	csr_clear(mstatus, MSTATUS_SPIE);
